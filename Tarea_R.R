...
# Primero creamos Edad_Sinistro, que es la diferencia entre el a�o en que ocurri� el siniestro y la edad de nacimiento
...
Base_2018 <- Base_2018[!(Base_2018$DEN_ASEG_BIN == 1 & Base_2018$CAU_DESCRIPCION == 'Defunci�n'),]
Base_Historica <- Base_Historica[!(Base_Historica$DEN_ASEG_BIN == 1 & Base_Historica$CAU_DESCRIPCION == 'Defunci�n'),]
...
Base_2018$SIN_ESTADO_ACTUAL[Base_2018$SIN_ESTADO_ACTUAL=="En Evaluaci¿n"]<-"En Evaluacion"
...
#Se separó la variable CAU_DESCRIPCION en 6 partes, CAUSA_Despido, CAUSA_Robo, CAUSA_Enfermedad, CAUSA_Clonacion, CAUSA_Incendio y CAUSA_Otros.
...
#Separando solo los casos de 'POSIBLE FRAUDE' (7.885 casos), se encontró que la participación de las causas anteriormente mencionadas respecto al universo de 7.885 casos es la siguiente:
#Despido: 76,94%
#Robo: 2,4%
#Enfermedad + Enfermedades Graves: 15,39%
#Clonacion: 1,47%
#Ademas se toma la variable Incendio como causa separada, ya que si bien no represente un porcentaje importante de los casos de 'POSIBLE FRAUDE' de la base sobre la que se trabaja, en el Manual Antifraude de la compañ�?a Caledonia Seguros se menciona como una causa importante de fraudes.
...
Base_2018$CAUSA_Otros = ifelse(Base_2018$CAU_DESCRIPCION == 'Despido', 0, ifelse(Base_2018$CAU_DESCRIPCION == 'Robo', 0, ifelse(Base_2018$CAU_DESCRIPCION == 'Enfermedad', 0, ifelse(Base_2018$CAU_DESCRIPCION == 'Clonación', 0, ifelse(Base_2018$CAU_DESCRIPCION == 'Incendio', 0,1)))))
...
#
#  	Creación otras variables sobre la misma Base_2018
#
...
#contratacion del seguro. En este caso, se asumirá que si la fecha de ocurrencia es anterior a la contratacion, la fecha
...
#Reducimos el número de obs. en MONTO_POR_DNII, eliminando todos los DNI que no están en Base_2018
Montos_Totales <- MONTO_POR_DNI %>%
  filter(MONTO_POR_DNI$DNI_ASEGURADO_LLAVE %in% Lista_DNI)
#Reducimos el número de obs. en N_POLIZAS_DNI, eliminando todos los DNI que no están en Base_2018
...
#Con la función "merge" generamos un left join, donde usamos la Base_2018 como base y cruzamos los datos con las otras bases generadas, lo
...
#Ahora TODOS los NA que quedaron en las nuevas columnas se deben igualar a 0, ya que como se mencionó más arriba, esto significa que estas
#personas no tienen registros históricos.
...
#Eliminamos las observaciones que cumplan con (Edad < 18 y DNI_ASEGURADO = DNI_DENUNCIANTE_LLAVE). Esto debido a que ningún menor de edad en
#Chile está habilitado para hacer cobros o hacer efectivos los seguros. En esos casos debe ser su tutor legal quien haga efectivo el seguro.
#Como no podemos saber si DNI_DENUNCIANTE_LLAVE  en los casos en que DNI_ASEGURADO != DNI_DENUNCIANTE_LLAVE es alguien mayor de edad, se asumirá que
...
#Procedemos a eliminar también las personas >100 años de la base. Este grupo etario es muy poco frecuente en Chile, ademas de existir en la base
...
#Queremos reducir la muestra total a una cantidad trabajable por nuestros computadores, lo cual está en torno a las 30.000 observaciones.
#Ademas queremos disminuir un poco la brecha en la proporcion de las clases para aumentar el Information Gain a pesar de aumentar la entrop�?a existente.
...
#N: Número de observaciones que quieremos tenga la nueva base
table(Base_balance$CLASE)
#NO: 21044 POSIBLE FRAUDE: 8956, es decir 29,85% de POSIBLE FRAUDE, que es muy cercano a lo que buscábamos.
...
#Creamos la variable CAUSA_Otros
Base_Prediccion$CAUSA_Otros = ifelse(Base_Prediccion$CAU_DESCRIPCION == 'Despido', 0, ifelse(Base_Prediccion$CAU_DESCRIPCION == 'Robo', 0, ifelse(Base_Prediccion$CAU_DESCRIPCION == 'Enfermedad', 0, ifelse(Base_Prediccion$CAU_DESCRIPCION == 'Clonación', 0, ifelse(Base_Prediccion$CAU_DESCRIPCION == 'Incendio', 0,1)))))
...
#Creación de conteoPolizas, conteoSiniestros, sumaDeMontos y Region para la Base_Prediccion
...
#Eliminamos los 7 valores que se duplicaron y la base queda tal como deber�?a estar:
Base_Full<- Base_Full[!duplicated(Base_Full$Posicion), ]

#Quitamos SIN_NOMBRE_CIUDAD y solo nos quedamos con Region y además quitamos el valor Posicion
Base_Full <- Base_Full %>%
...
#Extracción de la base train
Base_Full_ml_train <- Base_Full_ml%>%
  filter(is.na(Clase_bin)==FALSE)

#Extración de la base test // Esta es la base final que nos piden predecir
...
##En esta parte procedemos a realizar un análisis por correralacion entre las variables numéricas, con foco en Clase_bin
...
#Acá estamos diciendo que filtre: >=0.05 O <0.05 CUALQUIREA QUE CUMPLA UNA DE ESAS CONDICIONES SE QUEDA
...
#Agregamos la varaible no numérica a utilizar
...
#con 19 variables, en el codigo solo se conservó la parte que agrega las 19 variables a Base_preml_train 
...
#Separación en training y testing desde la Base_Full_ml_train
...
#Realizar la predicción con el mejor modelo (DT 19 variables training 70% y testing 30%)
Best_model <- model_DT
...
##Es decir, si se usa la columna de prediccion_final para reemplazar la variable CLASE en el Excel Tarea_2019S2_PrediccionRequerida, los valores no coincidirán.
## 
